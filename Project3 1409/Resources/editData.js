...
var createEditWin = function(sqlData) {
	...
	var addLabel = Ti.UI.createLabel({
		color: "#000",
		text: "Edit user " + sqlData.userName,
	...
	saveButton.addEventListener("click", function() {
		var userName = editInputFields[0].value;
		var lastName = editInputFields[2].value;
		
		if(addData.checkData(userName, lastName)) {
			...
			createDialog("Success", "User was updated");
	...
	deleteButton.addEventListener("click", function() {
		var deleteDialog = Ti.UI.createAlertDialog({
			title: "Delete user " + sqlData.userName + " " + sqlData.lastName + "?",
			buttonNames: [ "Yes", "No" ],
			cancel: 1
		});
		...
		deleteDialog.addEventListener("click", function(e) {
			if(e.index != e.source.cancel) {
				...
				createDialog("Success", "User was deleted");
	...
	createEditInputRow("Username *", "Username", sqlData.userName, Ti.UI.KEYBOARD_DEFAULT, true);
	createEditInputRow("First name", "First name", sqlData.firstName, Ti.UI.KEYBOARD_DEFAULT, true);
	createEditInputRow("Last name", "Last name", sqlData.lastName, Ti.UI.KEYBOARD_PHONE_PAD, false);
